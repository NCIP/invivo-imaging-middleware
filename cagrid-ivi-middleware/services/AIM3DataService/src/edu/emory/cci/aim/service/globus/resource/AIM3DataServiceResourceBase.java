...
import edu.emory.cci.aim.common.AIM3DataServiceConstants;
import edu.emory.cci.aim.stubs.AIM3DataServiceResourceProperties;
import edu.emory.cci.aim.service.AIM3DataServiceConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class AIM3DataServiceResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(AIM3DataServiceResourceBase.class);

	private AIM3DataServiceResourceConfiguration configuration;
    ...
    public AIM3DataServiceResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.dataservice.DomainModel getDomainModel(){
		return ((AIM3DataServiceResourceProperties) getResourceBean()).getDomainModel();
	}
	
	public void setDomainModel(gov.nih.nci.cagrid.metadata.dataservice.DomainModel domainModel ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(AIM3DataServiceConstants.DOMAINMODEL);
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((AIM3DataServiceResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(AIM3DataServiceConstants.SERVICEMETADATA);
	...
	public AIM3DataServiceResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (AIM3DataServiceResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadDomainModelFromFile() {
      if(getDomainModel()==null){
		try {
			...
			((AIM3DataServiceResourceProperties) this.getResourceBean()).setDomainModel((gov.nih.nci.cagrid.metadata.dataservice.DomainModel) Utils.deserializeDocument(dataFile.getAbsolutePath(),
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((AIM3DataServiceResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
