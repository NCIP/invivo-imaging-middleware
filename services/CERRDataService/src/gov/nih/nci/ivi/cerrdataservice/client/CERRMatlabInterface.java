...
public class CERRMatlabInterface {

	String url;
	public CERRMatlabInterface() {
		System.out.println("Constructor for MatlabInterfaceForCERR called now");
	}

	public CERRMatlabInterface(String dataSourceURL) {
		dataSourceURL = url;
	}
	...
	private static CQLQuery CERR2CQL(CERRObject cerrQuery)
	{
		...
		if(cerrQuery.getSubjectID().length() != 0) {
			System.out.println("The Subject ID is" + cerrQuery.getSubjectID());
			gov.nih.nci.cagrid.cqlquery.Attribute attr = new gov.nih.nci.cagrid.cqlquery.Attribute();
	...
	public CERRObject[] queryCERRDataService(CERRObject queryObject, String dataSourceURL)
	{
		try {
			System.out.println(ObjectSerializer.toString(queryObject, new QName(
					"gme://Imaging.caBIG/1.0/edu.osu.bmi.ivi.cerr", queryObject
							.getClass().getName())));
		...
		try {
			System.err.println("Exe Query: " + ObjectSerializer.toString(query,
	...
	public String retrieveCERRDataService(CERRObject queryObject, String dataSourceURL, String localDestination)
	{
		...
		try {
			...
			TransferServiceContextReference tscr = cerrDataService.retrieveCERRObjects(fcqlQuery);
			if(tscr != null)
			{
		...
		if(tclient != null)
		{
			System.out.println("trf client is null");
